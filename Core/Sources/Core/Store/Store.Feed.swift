...
extension Store {
	...
	public func add(feed: Feed, userInitiated: Bool = true) async {
		if await (
			try? queue.write {
				try Feed
	...
	///   - feed: Feed to remove
	///   - userInitiated: Only syncs is initiated by user to prevent feedback loops
	public func delete(feed: Feed, userInitiated: Bool = true) {
	...
	public func markAllAsRead(filter: Filter) {
	...
	public func fetch(feed: Feed? = nil) async {
		await fetch(
